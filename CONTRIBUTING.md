...
This project is AI-friendly by design. If you are using Claude, GitHub Copilot, Cursor, or any other AI assistant to contribute to this repository, please follow this rule:

**Always point your AI agent to [AGENTS.md](AGENTS.md) as the absolute source of truth.**

The `AGENTS.md` file at the root of the repository contains the definitive rules for:
- Architecture constraints (what is allowed in `src/core` vs test files)
- Import syntax rules (`.js` extensions, no Node or Deno globals in source)
- Testing style (BDD `describe`/`it`, `@std/assert`)
- Commit discipline (Conventional Commits)

A short pointer file also exists for each specific agent:
